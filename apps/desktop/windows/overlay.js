...
const FOCUS_FONT_SCALE = 0.72
const DEFAULT_CONTEXT_LINES = 1.05
const VOICE_MIN_CONTEXT_LINES = 1.35
const VOICE_MAX_CONTEXT_LINES = 2.2
...
function voiceContextLines() {
  const visibleLines = viewport.clientHeight / Math.max(1, lineHeightPx())
  return clamp(visibleLines * 0.22, VOICE_MIN_CONTEXT_LINES, VOICE_MAX_CONTEXT_LINES)
}

function effectiveFontSize() {
  const focusedSize = Math.round(baseFontSize * FOCUS_FONT_SCALE)
  return referenceFocus ? clamp(focusedSize, 16, 96) : baseFontSize
}

function applyEffectiveFontSize() {
  textEl.style.fontSize = `${effectiveFontSize()}px`
}

function anchorCharForCurrentLine() {
  if (!wordSpans.length) return null
  const targetTop = scrollY + lineHeightPx() * 1.1
  let candidate = wordSpans[0]
  for (const span of wordSpans) {
    if (span.offsetTop <= targetTop) candidate = span
    else break
  }
  return Number(candidate.dataset.start) || 0
}

function restoreLayoutAnchor(anchorChar, fallbackRatio) {
  measureAnchors()
  if (anchorChar != null) {
    const span = wordForChar(anchorChar)
    const target = lineTargetForSpan(span)
    if (target != null) {
      seekTo(target)
      return
    }
  ...
  seekTo(max * clamp(fallbackRatio || 0, 0, 1))
}

function renderReference(payload) {
  const active =
    payload && payload.active ? payload.active :
    payload && typeof payload.dataUrl === 'string' ? payload :
    null
  const dataUrl = active && typeof active.dataUrl === 'string' ? active.dataUrl : ''
  const wasFocused = referenceFocus
  const anchorChar = wasFocused === !!(payload && payload.referenceFocus)
    ? null
    : anchorCharForCurrentLine()
  const fallbackRatio = scrollY / Math.max(1, getMaxScroll())
  referenceFocus = !!(payload && payload.referenceFocus)
  applyEffectiveFontSize()

  if (dataUrl) {
    imageEl.src = dataUrl
    imagePanel.classList.add('visible')
    imagePanel.classList.toggle('focused', referenceFocus)
    imagePanel.classList.toggle('minimized', !referenceFocus)
    bodyEl.classList.toggle('reference-focused', referenceFocus)
    if (active.name) imageEl.alt = active.name
  } else {
    imageEl.removeAttribute('src')
    imageEl.alt = ''
    imagePanel.classList.remove('visible', 'focused', 'minimized')
    bodyEl.classList.remove('reference-focused')
  }

  requestAnimationFrame(() => {
    if (anchorChar == null) measureAnchors()
    else restoreLayoutAnchor(anchorChar, fallbackRatio)
    updateChapterBadge()
  })
}
...
function voiceTargetForChar(charPos) {
  const span = wordForChar(charPos)
  return lineTargetForSpan(span, { contextLines: voiceContextLines(), advanceLine: false })
}
...
function voiceTargetForWordIndex(index) {
  if (!wordSpans.length) return null
  const safeIndex = Math.max(0, Math.min(wordSpans.length - 1, Math.floor(index)))
  return lineTargetForSpan(wordSpans[safeIndex], {
    contextLines: voiceContextLines(),
    advanceLine: false
  })
}

function lineTargetForSpan(span, opts = {}) {
  if (!span) return null
  const lh = lineHeightPx()
  const contextLines = Math.max(
    0,
    Number.isFinite(Number(opts.contextLines)) ? Number(opts.contextLines) : DEFAULT_CONTEXT_LINES
  )
  const shouldAdvanceLine = opts.advanceLine !== false
  ...
  return Math.max(0, targetTop - lh * contextLines)
}
...
function nudgeScroll(direction, lines = 3) {
  const amount = lineHeightPx() * Math.max(1, Number(lines) || 1)
  const nextY = scrollY + Math.sign(Number(direction) || 0) * amount
  seekTo(nextY)
  if (voiceFollowActive) {
    voiceTargetY = scrollY
  }
...
window.cue.onManualScroll((data) => {
  if (!data) return
  nudgeScroll(data.direction, data.lines)
})
...
window.cue.onFontSet((v) => {
  baseFontSize = Math.min(96, Math.max(16, Number(v) || 32))
  const anchorChar = anchorCharForCurrentLine()
  const fallbackRatio = scrollY / Math.max(1, getMaxScroll())
  applyEffectiveFontSize()
  // re-measure on next frame (font changes line wrap)
  requestAnimationFrame(() => {
    restoreLayoutAnchor(anchorChar, fallbackRatio)
    updateChapterBadge()
  })
})
...
window.cue.onImageSet((dataUrl) => {
  renderReference({
    active: dataUrl && typeof dataUrl === 'string'
      ? { id: 'legacy-image', type: 'image', name: 'Reference image', dataUrl }
      : null,
    referenceFocus
  })
})

window.cue.onReferenceSet((payload) => renderReference(payload))
...
window.cue.onVoiceScroll((data) => {
  if (!data) return
  const max = getMaxScroll()
  const prevTargetY = voiceTargetY
  const prevScrollY = scrollY
  const prevWordIndex = voiceLastWordIndex
  let target
  let baseTarget = null
  let leadTarget = null
  if (typeof data.charPos === 'number') {
    setVoiceHighlight(data.charPos)
    baseTarget = voiceTargetForChar(data.charPos)
    target = baseTarget
  ...
  const wordAdvance = voiceLastWordIndex - prevWordIndex
  const targetDelta = target - prevTargetY
  if (wordAdvance >= 5 || targetDelta > lineHeightPx() * 1.8) {
    voiceLastDebugWordIndex = voiceLastWordIndex
    voiceDebug('overlay-target', {
      wordIndex: voiceLastWordIndex,
      wordAdvance,
      charPos: data.charPos,
      scrollY: Math.round(prevScrollY),
      baseTarget: baseTarget == null ? null : Math.round(baseTarget),
      leadTarget: leadTarget == null ? null : Math.round(leadTarget),
      target: Math.round(target),
      prevTargetY: Math.round(prevTargetY),
